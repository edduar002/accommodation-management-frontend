...
export class ChangePasswordHostComponent {
  // Objeto que almacena la contraseña actual y la nueva del anfitrión
   ...
   * @param _hostService Servicio para manejar operaciones del anfitrión
  ...
  onSubmit(form: NgForm): void {
    ...
    // Obtener datos del anfitrión desde localStorage
    const host = JSON.parse(localStorage.getItem('host') || '{}');

    // Asignar el id del anfitrión al objeto de recuperación de contraseña
    this.recoveryPassword.id = host.id || 2; // Valor por defecto en caso de no existir
